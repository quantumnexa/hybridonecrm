...
export default function AppointmentsPage() {
  ...
  const [cancelModal, setCancelModal] = useState({ open: false, apptId: null, leadId: null, reason: "", existingNotes: "" });

  const fetchAll = useCallback(
    async (uid) => {
      if (!uid) return;
      setError("");
      const { data: assignedLeads } = await supabase.from("leads").select("*").eq("sales_person", uid);
      const assignedIds = Array.from(new Set((assignedLeads || []).map((l) => l.id)));
      const { data: apptsByLead } =
        assignedIds.length > 0
          ? await supabase.from("appointments").select("*").in("lead_id", assignedIds)
          : { data: [] };
      const { data: apptsByActor } = await supabase.from("appointments").select("*").eq("created_by", uid);
      const map = new Map();
      (apptsByLead || []).forEach((a) => map.set(a.id, a));
      (apptsByActor || []).forEach((a) => map.set(a.id, a));
      const merged = Array.from(map.values()).sort((a, b) => new Date(a.scheduled_at) - new Date(b.scheduled_at));
      setAppointments(merged);
      const allLeadIds = Array.from(new Set(merged.map((a) => a.lead_id).filter(Boolean)));
      if (allLeadIds.length > 0) {
        const { data: apptLeads } = await supabase.from("leads").select("*").in("id", allLeadIds);
        setLeads(apptLeads || []);
      } else {
        setLeads([]);
      }
      const { data: leadsDataForCreate } = await supabase
        .from("leads")
        .select("*")
        .eq("status", "Appointment confirmed")
        .eq("sales_person", uid);
      setLeadsForCreate(leadsDataForCreate || []);
      const actorIds = Array.from(new Set(merged.map((a) => a.created_by).filter(Boolean)));
      if (actorIds.length > 0) {
        const { data: profiles } = await supabase.from("profiles").select("user_id, display_name, role").in("user_id", actorIds);
        const lab = {};
        (profiles || []).forEach((p) => (lab[p.user_id] = p.display_name || p.role || p.user_id));
        setActorLabels(lab);
      } else {
        setActorLabels({});
      }
  ...
  const createAppointment = async () => {
    ...
    if (data) {
      ...
      const leadIds = Array.from(new Set([...appointments.map((a) => a.lead_id), data.lead_id].filter(Boolean)));
      if (leadIds.length > 0) {
  ...
  const openCancelModal = (a) => {
    ...
    setCancelModal({ open: true, apptId: a.id, leadId: a.lead_id, reason: "", existingNotes: a.notes || "" });
  };
  ...
  const confirmCancel = async () => {
    ...
    if (data) {
      setAppointments((prev) => prev.map((x) => (x.id === cancelModal.apptId ? data : x)));
      await supabase
        .from("lead_activities")
        .insert({
          lead_id: cancelModal.leadId,
          type: "appointment_status_changed",
    ...
    setCancelModal({ open: false, apptId: null, leadId: null, reason: "", existingNotes: "" });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-black/10 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      {editId === a.id ? (
                        ...
                        <>
                          ...
                          <div className="text-xs text-black/60">
                            {new Date(a.scheduled_at).toLocaleString()} • {(actorLabels[a.created_by] || "Unknown")}
                          </div>
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        value={a.status === "scheduled" ? "Booked" : a.status === "completed" ? "Completed" : "Cancelled"}
                        onChange={(e) => {
            ...
            {appointments.length === 0 && <div className="text-sm text-black/60">No appointments found.</div>}
          </div>
        </div>
        {cancelModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="w-full max-w-lg rounded-xl border border-black/10 bg-white p-4 shadow-lg">
              ...
              <div className="mt-2 text-sm text-black/70">Please provide a reason for cancellation. It will be saved in the appointment notes.</div>
              <textarea
              ...
              <div className="mt-3 flex justify-end gap-2">
                <button className="rounded-md border border-black/10 px-3 py-2 hover:bg-black/5" onClick={() => setCancelModal({ open: false, apptId: null, leadId: null, reason: "", existingNotes: "" })}>Close</button>
