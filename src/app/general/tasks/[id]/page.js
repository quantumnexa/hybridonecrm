...
export default function GeneralTaskDetailPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (revTaskIds.length) {
      const { data: revTaskDocs } = await supabase
        .from("task_documents")
        .select("id,task_id")
        .in("task_id", revTaskIds);
      const counts = (revTaskDocs || []).reduce((acc, d) => {
        acc[d.task_id] = (acc[d.task_id] || 0) + 1;
        return acc;
      }, {});
    ...
    const ids = Array.from(
      new Set(
        [
          ...
          ...(revTasks || []).map((x) => x.created_by),
          ...(revTasks || []).map((x) => x.assignee_id),
        ].filter(Boolean)
      )
    );
    if (ids.length) {
  ...
  const createRevision = async () => {
    ...
    if (!canAddRevision) {
      setError("Only the task assigner can add revisions.");
      return;
    }
    setError("");
        ...
        <>
          ...
          <div className="rounded-xl border border-black/10 bg-white p-4 shadow-sm">
            ...
            {canAddRevision ? (
              ...
              <div className="mt-2 text-sm text-black/60">Only the task assigner can add revisions for this task.</div>
            )}
                  ...
                  <div key={r.id} className="rounded-md border border-black/10 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        <Link href={`/general/tasks/${r.id}`} className="font-semibold text-heading hover:underline">
                          {r.title}
                        </Link>
